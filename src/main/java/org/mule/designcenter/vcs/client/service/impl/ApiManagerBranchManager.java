...
public class ApiManagerBranchManager implements BranchFileManager {
    ...
    public ApiManagerBranchManager(String orgId, String userId, AccessTokenProvider provider, Branch branch) {
        this.orgId = orgId;
        this.userId = userId;
    ...
    public boolean delete(String path) {
        final ApiDesignerXapiResponse<Void> delete = branch.files.filePath(path).delete(new FilePathDELETEHeader(orgId, userId), provider.getAccessToken());
        return true;
    }

    @Override
    public boolean newFile(String path, byte[] content, String mimeType) {
        final List<FileContent> fileContents = Collections.singletonList(new FileContent(path, new String(content, ApiVCSConfig.DEFAULT_CHARSET)));
        final ApiDesignerXapiResponse<List<File>> post = branch.save.post(fileContents, new SavePOSTHeader(orgId, userId), provider.getAccessToken());
        return true;
    }
    ...
    public List<ApiFile> listFiles() {
        final List<File> fileList = branch.files.get(new FilesGETHeader(orgId, userId), provider.getAccessToken()).getBody();
    ...
    public ApiFileContent fileContent(String path) {
        final ApiDesignerXapiResponse<String> stringApiDesignerXapiResponse = branch.files.filePath(path).get(new FilePathGETHeader(orgId, userId), provider.getAccessToken());
        final String body = stringApiDesignerXapiResponse.getBody();
        return new ApiFileContent(body.getBytes(ApiVCSConfig.DEFAULT_CHARSET),stringApiDesignerXapiResponse.getResponse().getMediaType().toString());
    }
}
